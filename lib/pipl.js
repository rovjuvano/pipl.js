...
PIPL.Engine = (function() {
  ...
  Engine.prototype.enqueue = function(type, channelName, process, refs) {
    this.channels[channelName] || (this.channels[channelName] = {
      read: [],
      send: []
    });
    var channel = this.channels[channelName];
    channel[type].push([process, refs]);
  ...
  Engine.prototype.step = function() {
    ...
    var _;
    _ = select(channel.read);
    var reader = _[0], reader_refs = _[1];
    _ = select(channel.send);
...
PIPL.ReadProcess = (function(_super) {
  ...
  ReadProcess.prototype.proceed = function(refs) {
    this.engine.enqueue('read', refs.get(this.channel_id), this, refs);
  };
...
PIPL.SendProcess = (function(_super) {
  ...
  SendProcess.prototype.proceed = function(refs) {
    this.engine.enqueue('send', refs.get(this.channel_id), this, refs);
  };
